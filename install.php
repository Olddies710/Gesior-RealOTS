	...
	elseif($step == 2)
{
    ...
    try {
        if ($SQL->establishConnection()) {
            echo '<font color="green"><b>Database connection successful!</b></font><br>';
            echo 'Press <a href="install.php?page=step&step=3">link to STEP 3 - Add tables and columns to DB</a> to continue.';
        } else {
            echo '<font color="red"><b>Database connection failed!</b></font><br>';
            echo 'Error: ' . $SQL->connectionError . '<br>';
            echo 'Check your database configuration or consult the FAQ.';
        }
    } catch (Exception $e) {
        echo '<font color="red"><b>Error:</b></font> ' . $e->getMessage();
    }
}
	elseif($step == 3)
	{
