...
else
{
	if($action == "")
	{
        ...
        // Verificar que $account_players no sea null antes del foreach
        ...
        $main_content .= '</NOBR></td><td><NOBR>'.$account_player->getLevel().' '.htmlspecialchars($vocation_name[$account_player->getVocation()] ?? 'None').'</NOBR></td>';
        }
		...
		if($_POST['registeraccountsave'] == "1")
		{
			if($account_logged->isValidPassword($reg_password))
			{
				if(empty($old_key))
				{
                    ...
                    for($i=0; $i < 10; $i++) {
                    $cnum[$i] = $acceptedChars[mt_rand(0, $max)]; // Cambiado { } por [ ]
                    $new_rec_key .= $cnum[$i];
                }
...
{ 
		...
		{ 
			...
			if($savecharacter != 1)
            {
				...
				if(isset($config['site']['newchar_vocations']) && count($config['site']['newchar_vocations']) > 1)
				...
				if(isset($config['site']['newchar_towns']) && count($config['site']['newchar_towns']) > 1 || isset($config['site']['newchar_vocations']) && count($config['site']['newchar_vocations']) > 1)
				...
				if(count($config['site']['newchar_towns']) > 1)
				{
                  ...
                  $main_content .= '</table></td>'; // CORREGIDO: Solo un </table>
                }
                  if(isset($config['site']['newchar_towns']) && count($config['site']['newchar_towns']) > 1 || isset($config['site']['newchar_vocations']) && count($config['site']['newchar_vocations']) > 1)
